/**
 * This static class defines keys that can be stored in the window.sessionStorage
 */
export class CMS_SESSION_STORAGE_ITEM {
    static USER = "User";
    static DISPLAY = "Display";
    static SETTINGS = "Settings";
    static USER_LASTACTION_TIME = "UserLastActionTime";
    static DISPLAYS_SEARCH_FILTER = "DisplaysSearchFilter";
    static SOURCES_SEARCH_FILTER = "SourcesSearchFilter";
    static LAYOUTS_FAVORITE_FILTER = "LayoutsFavoriteFilter";
    static DISPLAYS_FAVORITE_FILTER = "DisplaysFavoriteFilter";
    static SOURCES_FAVORITE_FILTER = "SourcesFavoriteFilter";
}