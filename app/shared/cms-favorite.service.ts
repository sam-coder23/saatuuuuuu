...
export class CmsFavoriteService {
     ...
     * @return {void}
     */
    public markObjectAsFavorite(objectId: number, objectType: string, objectArray: any[]) {
        let objectTypePrefix = this.getObjectTypePrefix(objectType);
        if (objectTypePrefix === "") {
            this.appConfig.log(`CmsFavoriteService: Unable to find a valid prefix to call favorite 
            API.`);
            return;
        }
