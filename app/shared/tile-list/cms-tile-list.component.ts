...
export class CmsTileListComponent implements OnInit, OnDestroy {
    @Input() sourceCount: number = 0;
    @Input() displayResolution: { "width": number, "height": number };
     ...
     * @return {void}
     */
    public loadTilePreset(tilePreset: ITilePreset) {
    ...
    private selectTile(tilePreset) {
        ...
        for (let tileIndex = 0; tileIndex < this.tilePresets.length; tileIndex++) {
     ...
     * @return {void}
     */
    private selectTileInPresets(display: any) {
        if (!display) {
            return;
        };
