...
export class CmsMiniDisplayComponent implements OnInit, OnChanges, OnDestroy {
     ...
     * @returns {void}
     */
    private initDisplayTileInfoWithContent() {
    ...
    */
    ...
    private configurePinchZoomOnMiniDisplay(manager: HammerManager) {
        ...
        let enablePinch = function (event: TouchEvent) {
        ...
        let disablePinch = function (event: TouchEvent) {
            manager.get("pinch").set({
                enable: false
            });
        }
    ...
    private zoom(deltaY: number) {
        let currentWidth = parseInt(this.miniDisplayStyle.width);
        let currentHeight = parseInt(this.miniDisplayStyle.height);
        ...
        let newHeight = this.miniDisplaySize.height + (this.miniDisplaySize.height * this.zoomLevel / 100);
    ...
    private addMiniDisplayEventListeners(): void {
        ...
        let container: HTMLElement[] = this.domManager
