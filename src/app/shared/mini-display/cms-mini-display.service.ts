...
import { Observer, Observable, fromEvent, timer, throwError, debounce } from "rxjs";
...
export class CmsMiniDisplayService {
    ...
    public init(): void {
        ...
        this.windowResizeEndEvent = fromEvent(window, "resize").pipe(debounce(() =>
            timer(timeValue)
        ));
        this.panend = false;
    }
    ...
    public getMiniDisplayTilerInfoWithContent(aDisplayId: number, aContainer: HTMLElement)
        ...
        return Observable.create((observer: Observer<{}>) => {
            this.cmsServerApi.getSelectedDisplayContent(aDisplayId)
                ...
                (error: any) => {
                    throwError(() => `MiniDisplayComponent: Display detail
