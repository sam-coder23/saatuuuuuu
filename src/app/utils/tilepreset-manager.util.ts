...
export class TilePresetManager {
     ...
     */
    public static GET_TILE_ID (tilePresets: ITilePreset[], sourceCount: number,
        ...
        let tileIdIndex: number = tilePresets.findIndex((tilePreset: ITilePreset) =>
            tilePreset.noOfTiles === sourceCount && tilePreset.defaultForDisplays && (tilePreset.defaultForDisplays.indexOf(displayId) !== -1));

        if (tileIdIndex === -1) {
            // if all displays are set as default (generic) and no. of selected sources are equal to no. of tiles
            tileIdIndex = tilePresets.findIndex((tilePreset: ITilePreset) =>
                tilePreset.noOfTiles === sourceCount &&
